...
- Registers a `power_manager` sleep listener. On `PM_EVT_PREPARE_SLEEP`: leaves `AM_CLIENT_MUSIC` untouched (its PM lock and ALDO3 hold keep the CPU awake and audio supply live while the AMOLED panel DCS-sleeps), and only closes a `AM_CLIENT_NOTIFY` stream if one happens to be mid-play.
- Arbitrates between two client tiers: `AM_CLIENT_NOTIFY` (high priority) and `AM_CLIENT_MUSIC` (low priority). When a notification wants the codec while music is playing, music is paused, the notification plays, then music auto-resumes.
...
| Client | Priority | Used by |
|--------|----------|---------|
| `AM_CLIENT_MUSIC` | Low — preempted by NOTIFY | `music_player` |
| `AM_CLIENT_NOTIFY` | High — preempts MUSIC | `audio_alert` |

## Sleep behaviour

`AM_CLIENT_MUSIC` holds the PM no-sleep lock and ALDO3 rail while playing. When `power_manager_request_sleep()` fires:
...
- The AMOLED panel DCS-sleeps independently. Music plays through the dark display.
